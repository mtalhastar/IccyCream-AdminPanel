...
async function fetchDataFromFirestore() {
  const querySnapshot = await getDocs(collection(db, "icecreams"));
  const data = [];
...
const Page = () => {
  ...
  const deleteDataFromFirestore = async (docid) => {
    try {
      await deleteDoc(doc(db, "icecreams", docid));
      ...
      <div className="flex justify-center">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 mt-8">
          {filteredIcecreams && filteredIcecreams.map((icecream) =>
            <Card key={icecream.id} props={icecream} deletefunction={deleteDataFromFirestore} />
          )}
        </div>
      </div>
    </>
